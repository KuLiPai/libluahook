...
public:
	using SymbolResolver = void* (*)(void* module, const char* name);

	static auto SetSymbolResolver(SymbolResolver resolver) -> void {
	...
	static auto Invoke(const std::string& funcName, Args... args) -> Return {
		...
		if (address_.find(funcName) == address_.end() || !address_[funcName]) {
			address_[funcName] = symbol_resolver_
				? symbol_resolver_(hmodule_, funcName.c_str())
				: dlsym(hmodule_, funcName.c_str());
		}
#endif
...
private:
	inline static SymbolResolver symbol_resolver_{};
